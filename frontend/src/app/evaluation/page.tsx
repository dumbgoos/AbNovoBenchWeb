...
import { Target, Settings, Award, BarChart3, TrendingUp, Zap, RefreshCw } from 'lucide-react';
import AssemblyTables from '@/components/assembly/AssemblyTables';
import { ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ScatterChart, Scatter, LineChart, Line, ReferenceLine } from 'recharts';
  ...
  const fetchEvaluationData = async () => {
    try {
      ...
      const combinedData: EvaluationData = {
        models: evaluationResult.data?.models || [],
        datasets: evaluationResult.data?.datasets || [],
        metrics: evaluationResult.data?.metrics || [],
        last_updated: evaluationResult.data?.last_updated || new Date().toISOString(),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
              ...
              <div className="space-y-3">
                ...
                <Select value={selectedMetricType} onValueChange={(value) => {
                  ...
                  if (value === 'accuracy') {
                    ...
                    setSelectedMetric('assembly_scores');
                  }
                }}>
                ...
                <Select value={selectedMetric} onValueChange={(value) => {
                  ...
                  <SelectContent className="bg-white/95 backdrop-blur-sm">
                    ...
                    {selectedMetricType === 'assembly' && (
                      <>
                        <SelectItem value="assembly_scores" className="hover:bg-orange-50">Assembly Scores</SelectItem>
                        <SelectItem value="assembly_stats" className="hover:bg-orange-50">Assembly Statistics</SelectItem>
                        <SelectItem value="assembly_alps" className="hover:bg-orange-50">ALPS Results</SelectItem>
                      </>
                    )}
                  </SelectContent>
                </Select>
      ...
      {selectedMetricType === 'coverage_depth' && (
        ...
        {/* Model Selection for non-coverage depth and non-robustness and non-assembly metrics */}
        {selectedMetricType !== 'coverage_depth' && selectedMetricType !== 'robustness' && selectedMetricType !== 'assembly' && (
          <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl">
            <CardHeader className="bg-gradient-to-r from-green-50 to-blue-50 rounded-t-lg">
              <CardTitle className="flex items-center gap-3 text-xl">
        ...
        {/* Radar Chart for non-coverage depth and non-robustness and non-assembly metrics */}
        {selectedMetricType !== 'coverage_depth' && selectedMetricType !== 'robustness' && selectedMetricType !== 'assembly' && (
          <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 group">
            <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-t-lg">
              <CardTitle className="flex items-center gap-3 text-xl">
        ...
        {/* Bar Chart for non-coverage depth and non-robustness and non-assembly metrics */}
        {selectedMetricType !== 'coverage_depth' && selectedMetricType !== 'robustness' && selectedMetricType !== 'assembly' && (
          <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 group">
        ...
        {/* Assembly Metrics */}
        {selectedMetricType === 'assembly' && (
          <AssemblyTables selectedModels={selectedModels} selectedIndicator={selectedMetric} />
        )}
